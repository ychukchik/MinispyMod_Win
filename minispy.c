...
#pragma warning(pop)
        ...
        DbgPrintEx(0, 0, "[Debug_print] Start read config\n");
        ...
        if (NT_SUCCESS(status)) {
            status = ZwReadFile(fileHandle, NULL, NULL, NULL, &iostatus, buffer_data, 1024, &byteOffset, NULL);

            int pos = 0;
            for (char* c = buffer_data; *c; c++) {
                int i = 0, j = 0;
                if (!(*c) || *c == '\r')
                    break;
                rule* rule_ = &rules[pos];
                while (*c != ' ')
                    rule_->file_name[j++] = *c++;
                rule_->file_name[j] = 0;
                c++;
                ...
                c += 2;
                pos++;
                DbgPrintEx(0, 0, "[Debug_print] %ws %ws %d\n", rule_->file_name, rule_->proc_name, rule_->access);
            }
            num_of_rules = pos;
            DbgPrintEx(0, 0, "[Debug_print] The config was read successfully. %d\n", num_of_rules);
            ZwClose(fileHandle);
        }
        else {
            DbgPrintEx(0, 0, "[Debug_print] Error open config.\n");
            ZwClose(fileHandle);
...
NTSTATUS GetProcessNameByHandle(PEPROCESS eProcess, PUNICODE_STRING* ProcessImageName) {
    NTSTATUS status = STATUS_SUCCESS;
    ...
    *ProcessImageName =
        ExAllocatePoolWithTag(NonPagedPoolNx, returnedLength, '2gat');
...
--*/
{
    ...
    NTSTATUS status_ = FltGetFileNameInformation(Data, FLT_FILE_NAME_NORMALIZED | FLT_FILE_NAME_QUERY_DEFAULT, &FileNameInfo);
    if (NT_SUCCESS(status_)) {
        status_ = FltParseFileNameInformation(FileNameInfo);
        if (NT_SUCCESS(status_)) {
            if (FileNameInfo->Name.MaximumLength < 512) {
                RtlCopyMemory(FileName, FileNameInfo->Name.Buffer, FileNameInfo->Name.Length);
                WCHAR buffer[512] = { 0 };
                PUNICODE_STRING processName = NULL;
                GetProcessNameByHandle(IoThreadToProcess(Data->Thread), &processName);
                RtlCopyMemory(buffer, processName->Buffer, processName->MaximumLength);
                int mode = 1, flag = 1;
                //if ((wcsstr(FileName, L"\\Device\\HarddiskVolume3\\MBKS\\") != NULL)) { //&& (wcsstr(buffer, L"\\Device\\HarddiskVolume3\\MBKS\\") != NULL)
                for (int i = 0; i < num_of_rules; i++) {
                    if ((wcsstr(FileName, rules[i].file_name) != NULL) && (wcsstr(buffer, rules[i].proc_name) != NULL)) {
                        mode = rules[i].access;
                        DbgPrintEx(0, 0, "[Debug_print] rule %ws for %ws access %d\n", rules[i].proc_name, FileName, rules[i].access);
                        if ((Data->Iopb->MajorFunction == IRP_MJ_READ && mode == 2) || (Data->Iopb->MajorFunction == IRP_MJ_WRITE && mode == 1)) {
                            DbgPrintEx(0, 0, "[Debug_print] ERROR: %ws for %ws blocked\n", rules[i].proc_name, FileName);
                ...
                //if (wcsstr(FileName, L"config.txt") != NULL) {
                //    OBJECT_ATTRIBUTES oa;
                //    UNICODE_STRING    fullFileName;
                //    HANDLE            fileHandle;
                //    IO_STATUS_BLOCK   iostatus;
                ...
                //    DbgPrintEx(0, 0, "[Debug_print] Start update config\n");
                ...
                //        NULL,
                //        NULL);
                //    DbgPrintEx(0, 0, "[Debug_print] Create file\n");
                //    status = ZwCreateFile(&fileHandle,
                //        GENERIC_READ,
                //        &oa,
                //        &iostatus,
                ...
                //        NULL,
                //        0);
                //    DbgPrintEx(0, 0, "[Debug_print] Check statuc\n");
                //    if (NT_SUCCESS(status)) {
                //        DbgPrintEx(0, 0, "[Debug_print] Read file\n");
                //        status = ZwReadFile(fileHandle, NULL, NULL, NULL, &iostatus, buffer_data, 1024, &byteOffset, NULL);
                ...
                //        }
                //        num_of_rules = pos;
                //        DbgPrintEx(0, 0, "[Debug_print] The config was update successfully. %d\n", num_of_rules);
                //        ZwClose(fileHandle);
                //    }
                //    else {
                //        DbgPrintEx(0, 0, "[Debug_print] Error open config.\n");
                //        ZwClose(fileHandle);
                //    }
                //    DbgPrintEx(0, 0, "[Debug_print] UPDATED!\n");
                //}
                /*if (flag) {
                    DbgPrintEx(0, 0, "[Debug_print] ERROR: The process isn't in the config.txt\n");
                    Data->IoStatus.Status = STATUS_ACCESS_DENIED;
                    Data->IoStatus.Information = 0;
                    FltReleaseFileNameInformation(FileNameInfo);
                    return FLT_PREOP_COMPLETE;
            ...
            if (FileNameInfo->Name.MaximumLength < 512) {
                ...
                if (wcsstr(FileName, L"config.txt") != NULL) {
                    ...
                    InitializeObjectAttributes(&oa,
                        ...
                        NULL);
                    DbgPrintEx(0, 0, "[Debug_print] Create file\n");
                    NTSTATUS status = ZwOpenFile(&fileHandle, GENERIC_READ, &oa, &iostatus, FILE_SHARE_READ | FILE_SHARE_WRITE,
                        FILE_SYNCHRONOUS_IO_NONALERT);
                    DbgPrintEx(0, 0, "[Debug_print] Handle: %d\n", fileHandle);
                    DbgPrintEx(0, 0, "[Debug_print] Check statuc\n");
                    if (NT_SUCCESS(status)) {
                        DbgPrintEx(0, 0, "[Debug_print] Read file\n");
                        status = ZwReadFile(fileHandle, NULL, NULL, NULL, &iostatus, buffer_data, 1024, &byteOffset, NULL);
